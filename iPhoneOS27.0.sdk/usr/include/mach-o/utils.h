...
/*!
 * Converts an architecture name into a cpu type/subtype pairy.
 *
 * - Parameter archName: An architecture name (e.g "arm64e" or "x86_64").
 * - Parameter type:     A pointer to where to store the cpu type of the given name.
 * - Parameter subtype:  A pointer to where to store the cpu subtype of the given name.
 *
 * - Returns: Returns true if the archName is known and fills in the type/subtype.
 *            Returns false if the archName is unknown.
 *
 */
 ...
 *
 * - Parameter type:     The cpu type from <machine/machine.h> (e.g CPU_TYPE_ARM64)
 * - Parameter subtype:  The cpu subtype from <machine/machine.h> (e.g CPU_SUBTYPE_ARM64E)
 *
 * - Returns: Returns a static c-string which is the name for the cpu type/subtype (e.g. "arm64e").
 ...
 *
 * - Parameter mh: A pointer to the header of a mach-o file.
 *                 If NULL is passed, the architecture name of the main executable will be returned.
 *
 * - Returns: Returns a static c-string which is the name for architecture of the mach-o file (e.g. "arm64e").
...
/*!
 * Iterates slices in a universal file given its path.
 *
 ...
 * If the file is a universal static library, the block is called with a pointer to the
 * start of each static library, which is not a mach-o file! If you only want to inspect mach-o slices,
 * you can use `macho_arch_name_for_mach_header(slice) == NULL` to filter out non-mach-o files (e.g. static libs).
 * If the path does not exist or does, but is not a mach-o file, the block is never called.
 *
 * - Parameter path:     The path to the file to inspect.
 * - Parameter callback: A block to call once per slice.
 *                       This parameter can be NULL, in which case the return value tells you if the file is mach-o or fat.
 *                       The slice pointer is only valid during the block invocation.
 *                       To stop iterating the slices, set *stop to true.
 *
 * - Returns: Returns zero on success, otherwise it returns an errno value:
...
/*!
 * Iterates slices in a universal file given a file descriptor to the file.
 *
 * Temporarily maps a mach-o or universal file and iterates the slices.
 ...
 * If the fd is a universal static library, the block is called with a pointer to the
 * start of each static library, which is not a mach-o file! If you only want to inspect mach-o slices,
 * you can use `macho_arch_name_for_mach_header(slice) == NULL` to filter out non-mach-o files (e.g. static libs).
 * If the fd is closed or not mmap()able, the block is never called.
 *
 * - Parameter fd:       An open file descriptor to a mmap()able file.
 * - Parameter callback: A block to call once per slice.
 *                       This parameter can be NULL, in which case the return value tells you if the file is mach-o or fat.
 *                       The slice pointer is only valid during the block invocation.
 *                       To stop iterating the slices, set *stop to true.
...
/*!
 * Examines a mach-o or universal file by path to find the slice that would be loaded.
 *
 * For dylib/bundles, the best slice is the slice dyld would load.
 * For main executables, the best slice is the slice the kernel would execute.
 ...
 *
 * - Parameter path:      The path to the file to inspect.
 * - Parameter bestSlice: This block will be called once with the best slice.
 *                        This parameter can be NULL, in which case the return value tells you if there was a loadable slice.
 *                        The slice pointer is only valid during the block invocation.
 *
 * - Returns: Returns zero on success (meaning there is a best slice), otherwise it returns an errno value.
 *   * ENOENT    - path does not exist
 *   * EACCES    - path exists put caller does not have permission to access it
 *   * EFTYPE    - path exists but it is not a mach-o or fat file
...
/*!
 * Examines a mach-o or universal file by descripotr to find the slice that would be loaded.
 *
 * For dylib/bundles, the best slice is the slice dyld would load.
 * For main executables, the best slice is the slice the kernel would execute.
 * In simulator processes, only other simulator main executables will be considered loadable.
