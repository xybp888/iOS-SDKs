...
[[__noreturn__]] inline _LIBCPP_HIDE_FROM_ABI void __throw_bad_array_new_length() {
  ...
  // Workaround to prevent having to link against `libc++.dylib`. While all users should be
  // linking against it, in practice this can be tricky and can require a migration period.
  ::abort();
#endif
}
