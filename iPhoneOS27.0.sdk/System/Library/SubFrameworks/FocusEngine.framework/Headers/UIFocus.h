...
#if UIFOCUS_SUPPORTS_UIKIT
@class UIView, UIFocusEffect, UIFocusAnimationCoordinator, UIFocusMovementHint;
#endif
...
typedef NSString * UIFocusSoundIdentifier NS_TYPED_EXTENSIBLE_ENUM API_UNAVAILABLE(watchos, macos);
...
/// to the parent.
- (nullable UIFocusSoundIdentifier)soundIdentifierForFocusUpdateInContext:(UIFocusUpdateContext *)context API_AVAILABLE(tvos(11.0)) API_UNAVAILABLE(ios, watchos, visionos, macos);
...
#if UIFOCUS_SUPPORTS_UIKIT
UIKIT_EXTERN NSString * const UIFocusUpdateAnimationCoordinatorKey API_AVAILABLE(ios(11.0), tvos(11.0)) API_UNAVAILABLE(watchos) API_UNAVAILABLE(macos) NS_SWIFT_NAME(UIFocusSystem.animationCoordinatorUserInfoKey);
#endif // UIFOCUS_SUPPORTS_UIKIT
